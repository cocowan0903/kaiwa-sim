...
const LS_KEYS = {
  history: "dr:v3:history",
  favs: "dr:v3:favs",
  gateLastDate: "dr:v3:gate:lastDate",
};

const SS_KEYS = {
  gateShownThisSession: "dr:v3:gate:shown",
};
...
function normalizeActions(raw) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    // modes 欠損耐性（未指定は両対応）
    const modes = safeArr(a.modes).map(String);
    const modeTag = modes.length ? modes : ["general", "student"];
...
function Pill({ active, children, onClick }) {
  return (
    <button className={`pill ${active ? "isActive" : ""}`} onClick={onClick} type="button">
      {children}
    </button>
  );
}
...
export default function App() {
  ...
  // ✅ 履歴は「オブジェクト」で統一（React #31 事故防止）
  const [history, setHistory] = useState(() => {
    const raw = localStorage.getItem(LS_KEYS.history);
    const arr = safeArr(safeJsonParse(raw, []));
    // 変な形が混ざってても落ちないようにフィルタ
    return arr
      .filter((h) => h && typeof h === "object" && h.item && typeof h.item.title === "string")
      .slice(0, LIMITS.historyMax);
  });

  // ✅ favsは id の配列で保存 → 画面では Set にする
  ...
  // Gate（URLを開いた時だけ + 1日1回）
  const [gateOpen, setGateOpen] = useState(false);

  // Right: ranking（SideGame用）
  ...
  /** Ranking 読み込み（SideGame: mode="sidegame"） */
  useEffect(() => {
    let alive = true;
  ...
  const favList = useMemo(() => {
    const set = favs;
    return ACTIONS.filter((a) => set.has(String(a.id)));
  }, [ACTIONS, favs]);
    ...
    <div className="appShell">
      ...
      <div className="layout">
                  ...
                  {history.map((h) => (
                    ...
                    >
                      ...
                      <div className="liMeta">
                        {h.cond?.time}分 / {h.cond?.goal} / {h.cond?.place} / {h.cond?.money} /{" "}
                        {h.cond?.mode}
                      </div>
                    </button>
                  ))}
        ...
        <aside className="aside">
          ...
          <div className="card">
            <div className="rowBetween">
              <div className="cardTitle">🏆 ランキング（SideGame）</div>
              <button className="btn ghost" onClick={() => setRankTick((n) => n + 1)} type="button">
                更新
              </button>
            </div>
            ...
            <div className="smallNote muted">
              ※ best_scores の <code>mode="sidegame"</code> を Top{LIMITS.rankingMax}表示。<br />
              ※ 読めない場合は RLS（select許可）がまだ。
            </div>
          </div>
        </aside>
      </div>
